 ...
 *
 * This class serves all responsibility of starting and stopping distributed tests.这个类负责启动和停止分布式测试。
 * It was refactored from JMeter and RemoteStart classes to unify retry behavior.它是从JMeter和RemoteStart类重构的，以统一重试行为
 *
...
public class LocalDistributedRunner {
        ...
        for (int tryNo = 0; tryNo < retriesNumber; tryNo++) {
            ...
            while (idx < addrs.size()) {
                ...
                if (weight != null && weight > 0 && weight != 100) {
                    ...
                    //false 是如果test Element的类型node,也会clone.
    ...
    /**
     * Starts a remote testing engiens
     *
    ...
    private JMeterEngine getClientEngine(String address, HashTree testTree) {
        ...
        try {
            ...
            JMeterUtils.reportErrorToUser(ex.getMessage(),JMeterUtils.getResString("remote_error_init") + ": " + address); // $NON-NLS-1$ $NON-NLS-2$
            return null;
        }
    }
     ...
     */
    protected JMeterEngine createEngine(String address) throws NotBoundException, RemoteException {
