...
//            logger.info("casePath:" + casePath);
     ...
     */
    public void excuteJmeterRunByScript(StressTestFileEntity stressTestFile, StressTestReportsEntity stressTestReports, Map map) {
        //由于调试模式需要动态修改则测试结果保存样式为JTL,而性能测试报告需要的格式为CSV(压测系统默认模式)。
        //所以调试模式下如果运行脚本而无法生成测试报告，造成系统功能缺失。
        ...
        try {
            ...
            FileExecuteResultHandler resultHandler =
                    new FileExecuteResultHandler(stressTestFile, stressTestReports, this, outputStream, errorStream);
            //执行脚本命令
    ...
    /**
     * 本地执行Jmeter的脚本文件，采用apatch-Jmeter-Api来执行。
     */
    public void excuteJmeterRunLocal(StressTestFileEntity stressTestFile, StressTestReportsEntity stressTestReports, Map map) throws RuntimeException {
        ...
        try {
            ...
            //默认的FileServer.getFileServer().setBaseForScript(jmxFile);方法无法支持单进程内的
            //多脚本的同时进行，是Jmeter源生的对单进程内多执行脚本支持就不好。
