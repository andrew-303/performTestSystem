...
public class DebugTestReportsServiceImpl implements DebugTestReportsService {
    ...
    public void createReport(Long reportId) {
        ...
        //从文件构造StreamResult，充当转换结果的持有者，转换结果可以是XML、纯文本、HTML或其他形式的标记
        ...
        try {
            TransformerFactory tFactory = TransformerFactory.newInstance();//创建一个新的工厂实例
            Transformer transformer = tFactory.newTransformer(xsltSource);//返回一个transformer对象，该对象可用于在单个线程中执行转换
            transformer.transform(srcJtl,destResult);//将srcJtl转换为destResult
        } catch (Exception e) {
            //保存状态，执行出现异常
